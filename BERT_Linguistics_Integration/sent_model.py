...
def load_all_embeddings(directory, num_files):
    all_embeddings, all_labels = [], []
    max_length = 0
    for i in range(1, num_files + 1):
        path = os.path.join(directory, f'batch_{i}.pt')
        embeddings, labels = load_embeddings(path)
        max_length = max(max_length, embeddings.size(1))
    for i in range(1, num_files + 1):
        path = os.path.join(directory, f'batch_{i}.pt')
        embeddings, labels = load_embeddings(path)
        embeddings = pad_embeddings(embeddings, max_length)
        all_embeddings.append(embeddings)
        all_labels.append(labels)
    return torch.cat(all_embeddings), torch.cat(all_labels)

train_embeddings, train_labels = load_all_embeddings('/Users/ishaansingh/Documents/CS224N_FinalProject/sent_train', 85)
test_embeddings, test_labels = load_all_embeddings('/Users/ishaansingh/Documents/CS224N_FinalProject/sent_test', 10)
...
for epoch in range(20):
    model.train()
    epoch_losses = []
    for data, target in tqdm(train_loader, desc=f'Training Epoch {epoch+1}/20'):
...
with torch.no_grad():
    for data, target in tqdm(test_loader, desc='Testing'):
        ...
        total += target.size(0)
        correct += (predicted[:, 0] == target).sum().item()
