...
file_path = "/Users/ishaansingh/Documents/CS224N_FinalProject/presup_SARC_test.csv"
SARC_df = pd.read_csv(file_path)
SARC_df['presupposition'] = SARC_df['presupposition'].astype(str)

tqdm.pandas(desc="Generating Presupposition Embeddings")

base_dir = '/Users/ishaansingh/Documents/CS224N_FinalProject/SARC_presup_Embeddings_test'
batch_size = 10000
for i in tqdm(range(0, len(SARC_df), batch_size), desc="Processing Batches"):
    batch_df = SARC_df.iloc[i:i+batch_size]
    batch_df['embeddings'] = batch_df['presupposition'].progress_apply(lambda text: generate_embeddings(text))
    batch_df.dropna(subset=['embeddings'], inplace=True) 
    
    # a new directory for each batch
