...
def process_directory(presup_dir, naive_dir, implic_dir, output_dir_svd, output_dir_cat, output_dir_add, max_length=512):
    ...
    implic_files = [os.path.join(implic_dir, file_name) for file_name in sorted(os.listdir(implic_dir))]
    os.makedirs(output_dir_svd, exist_ok=True)
    os.makedirs(output_dir_cat, exist_ok=True)
    os.makedirs(output_dir_add, exist_ok=True)
    print(len(presup_files))
    for i, (presup_file, naive_file, implic_file) in enumerate(tqdm(zip(presup_files, naive_files, implic_files), total=len(presup_files), desc="Processing files")):
        ...
        implic_data = torch.load(implic_file + '/embeddings.pt', map_location = device)
        
        presup_embeddings = presup_data['features'] 
        naive_embeddings = naive_data['features'] 
        labels = naive_data['labels'] 
        implic_embeddings = implic_data['features'] 
        ...
        torch.save({'embeddings': combined_svd, 'labels': labels}, os.path.join(svd_path, file_name))
        torch.save({'embeddings': combined_concat, 'labels': labels}, os.path.join(concat_path, file_name))
        torch.save({'embeddings': combined_adding, 'labels': labels}, os.path.join(add_path, file_name))


if __name__ == '__main__':
    presup_dir = '/Users/ishaansingh/Documents/CS224N_FinalProject/SARC_presup_Embeddings_test'
    naive_dir = '/Users/ishaansingh/Documents/CS224N_FinalProject/SARC_Embeddings_test'
    implic_dir = '/Users/ishaansingh/Documents/CS224N_FinalProject/SARC_imp_Embeddings_test'
    output_dir_svd = '/Users/ishaansingh/Documents/CS224N_FinalProject/SARC_total_embeddings_svd_test'
    output_dir_cat = '/Users/ishaansingh/Documents/CS224N_FinalProject/SARC_total_embeddings_cat_test'
    output_dir_add = '/Users/ishaansingh/Documents/CS224N_FinalProject/SARC_total_embeddings_add_test'
    max_length = 512  
