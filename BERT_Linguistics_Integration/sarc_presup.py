...
for text in tqdm(SARC_df['response_text'], desc='Generating Presuppositions'):
    input_ids = tokenizer(text, return_tensors='pt').input_ids.to(device)
    output_ids = model.generate(input_ids)
    output_text = tokenizer.decode(output_ids[0], skip_special_tokens=True)
    outputs.append(output_text)

outputs_df = pd.DataFrame({'presupposition': outputs})
csv_output_path = '/Users/ishaansingh/Documents/CS224N_FinalProject/presup_SARC_train.csv'  # Added .csv to make the path a valid csv file path
