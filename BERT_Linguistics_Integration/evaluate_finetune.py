...
train_dataset_path = "split_data/train_presuppositions.jsonl"
val_dataset_path = "split_data/val_presuppositions.jsonl"
test_dataset_path = "split_data/test_presuppositions.jsonl"

model_name = "t5-base-finetuned-presuppositions-small"
base_name = "t5-small"
...
for dataset in [tokenized_train_dataset, tokenized_val_dataset, tokenized_test_dataset]:
    ...
    for i in range(0, total_samples, chunk_size):
        train_chunk = dataset.select(range(i, min(i + chunk_size, total_samples)))

        with torch.no_grad():
            chunk_metrics = eval_trainer.evaluate(train_chunk, metric_key_prefix=f"train_chunk_{i}")
        print(chunk_metrics)
        combined_bleu_scores.append(chunk_metrics[f"train_chunk_{i}_bleu"])
        combined_rouge_scores.append(chunk_metrics[f'train_chunk_{i}_rouge'])
        gc.collect()
    ...
    print(f"{dataset} Metrics (Combined):")
